...
-- NOTE: format('%I', 'public."X"') would mis-quote the qualified name, so each branch
-- uses a plain ALTER with explicit (safe, constant) identifiers instead.
DO $$ BEGIN
  IF to_regclass('public.product_variants') IS NOT NULL THEN
    EXECUTE 'ALTER TABLE public.product_variants ADD COLUMN IF NOT EXISTS "supplierVariantId" TEXT,
  ...
  ELSIF to_regclass('public."ProductVariant"') IS NOT NULL THEN
    EXECUTE 'ALTER TABLE public."ProductVariant" ADD COLUMN IF NOT EXISTS "supplierVariantId" TEXT,
      ADD COLUMN IF NOT EXISTS "supplierAttributes" JSONB,
      ADD COLUMN IF NOT EXISTS "supplierImage" TEXT,
      ADD COLUMN IF NOT EXISTS "supplierCostUsd" DOUBLE PRECISION,
      ADD COLUMN IF NOT EXISTS "supplierShippingUsd" DOUBLE PRECISION,
