/**
 * Normalizacion de presentacion para variantes AliExpress.
 * SOLO presentacion: limpia SKUs tecnicos y agrupa por atributo.
 * NO toca precios, margenes, envios, categorias, CJ, checkout ni pedidos.
 */
...
export function translateColorValue(cleaned: string): string {
  ...
  if (parts.length <= 2 && COLOR_ES[last] && !/\d/.test(key)) {
    ...
    if (parts[0] && /^(ipad|iphone|galaxy|tablet)$/.test(parts[0])) return label;
  }
  return s;
}
...
/**
 * Presentacion segura de un valor de modelo (solo patrones inequivocos;
 * si duda, devuelve el texto limpio original). Idempotente.
...
export function presentModelValue(value: string): string {
  ...
  const years: string[] = [];
  while (words.length && /^(19|20)\d{2}(?:[-/](?:19|20)?\d{2})?$/.test(words[words.length - 1] as string)) {
    years.unshift(words.pop() as string);
  }

  // Tamano al final: 10.2in / 10.2" / 10.9 (decimal o con unidad).
  let size = '';
  const lastWord = words[words.length - 1] ?? '';
  const sizeM = lastWord.match(/^(\d{1,2}(?:\.\d{1,2})?)(?:in(?:ch)?|["”])?$/i);
  if (sizeM && sizeM[1] && (/\d\.\d/.test(lastWord) || /in(?:ch)?|["”]/i.test(lastWord))) {
    size = sizeM[1];
    words.pop();
  }
  if (!words.length) return s;
  ...
  for (const w of words) {
    ...
    if (ord && ord[1]) { tokens.push({ kind: 'num', text: ord[1] }); continue; }
  ...
  for (let i = 0; i < tokens.length - 3; i++) {
    const a = tokens[i] as { kind: string; text: string };
    const b = tokens[i + 1] as { kind: string; text: string };
    const c = tokens[i + 2] as { kind: string; text: string };
    const d = tokens[i + 3] as { kind: string; text: string };
    if (a.kind === 'word' && b.kind === 'num'
      && c.kind === 'word' && d.kind === 'num'
      && a.text.toLowerCase() === c.text.toLowerCase()) {
      const nums = [b.text, d.text];
      let j = i + 4;
      while (j + 1 < tokens.length) {
        const wj = tokens[j] as { kind: string; text: string };
        const nj = tokens[j + 1] as { kind: string; text: string };
        if (wj.kind !== 'word' || wj.text.toLowerCase() !== a.text.toLowerCase() || nj.kind !== 'num') break;
        nums.push(nj.text);
        j += 2;
      }
      tokens.splice(i, j - i,
        { kind: 'word', text: a.text },
  ...
  for (const t of tokens) {
    ...
    if (t.kind === 'num' && prev && prev.kind === 'num') {
  ...
  segments.forEach((seg, idx) => {
    ...
    const prev = segments[idx - 1] as { kind: string; text: string };
...
/** Presentacion de un valor de atributo: Color -> espanol, Modelo -> formato seguro. */
export function presentAttributeValue(name: unknown, value: unknown): string {
  ...
  if (/^modelo/i.test(attr)) return presentModelValue(v);
  return v;
}
...
export function inferAttributeName(rawName: unknown, cleanedValue: string): string {
  ...
  if (!generic || /^(prop|attr|attribute|option|spec|sku|\d+)$/i.test(generic)) return 'Modelo';
...
export function toCommercialAttribute(rawName: unknown, rawValue: unknown): CleanAttribute | null {
  ...
  // "for iPad black" (prop sin nombre) mezcla modelo+color: extraer color final.
...
/** "iPad black" / "iPad green" -> "Negro" / "Verde" (mezcla modelo+color). Null si no aplica. */
...
export function extractAttributes(variantJson: any): CleanAttribute[] {
  const out: CleanAttribute[] = [];
  const structured: any[] = Array.isArray(variantJson?.attributes) ? variantJson.attributes : [];
  if (structured.length) {
    structured.forEach((a: any, idx: number) => {
      const c = toCommercialAttribute(a?.name ?? 'a' + idx, a?.value);
      if (c && c.value) out.push(c);
    });
    if (out.length) return fixDupes(out);
  }
  parseSkuAttr(variantJson?.skuAttr ?? variantJson?.sku ?? '').forEach((s, idx) => {
    const c = toCommercialAttribute(s.rawName || 'a' + idx, s.rawValue);
    if (c && c.value) out.push(c);
  });
  return fixDupes(out);
}
...
export function normalizeVariant(raw: any): NormalizedVariant {
  const json = raw?._json ?? {};
  const attrs = extractAttributes({
    attributes: raw?.supplierAttributes ?? json?.attributes,
    skuAttr: json?.skuAttr ?? raw?.skuAttr ?? raw?.sku ?? '',
    sku: raw?.sku ?? '',
  });
  return {
    id: String(raw?.id ?? raw?.supplierVariantId ?? raw?.sku ?? ''),
    sku: String(raw?.sku ?? raw?.supplierVariantId ?? ''),
    supplierVariantId: raw?.supplierVariantId ? String(raw.supplierVariantId) : undefined,
    price: Number(raw?.price ?? json?.salePriceClp ?? 0) || 0,
    stock: Number(raw?.stock ?? 0) || 0,
    image: json?.image ?? raw?.supplierImage ?? raw?.image ?? undefined,
    supplierCost: raw?.supplierCostUsd ?? json?.costUsd ?? null,
    supplierShipping: raw?.supplierShippingUsd ?? json?.shippingUsd ?? null,
    attributes: attrs,
  };
}
export function groupAttributes(variants: NormalizedVariant[]): AttributeGroup[] {
  ...
  const rank = (n: string) => (n === 'Modelo' ? 0 : n === 'Color' ? 1 : n === 'Talla' ? 2 : 3);
