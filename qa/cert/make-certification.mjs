...
/**
 * Cuenta checks de un log de `node --test`. Node 24 imprime el resumen con
 * "ℹ tests N"; versiones anteriores usan "# tests N". Se aceptan ambos.
 */
function suite(log) {
  const g = (re) => { const m = [...log.matchAll(re)].pop(); return m ? Number(m[1]) : null; };
  const tests = g(/(?:#|ℹ)\s*tests (\d+)/g);
  if (tests === null) return null;
  return { tests, pass: g(/(?:#|ℹ)\s*pass (\d+)/g), fail: g(/(?:#|ℹ)\s*fail (\d+)/g), skipped: g(/(?:#|ℹ)\s*skipped (\d+)/g) };
}
...
const pendientes = [
  ...
  'Publicacion y URL publica: no ejecutada (depende de la suscripcion anterior).',
...
const out = {
  ...
  estado: 'PARCIAL',
  nota: 'Las fases ejecutadas tienen evidencia real. Lo no ejecutado queda en `pendientes`; no se declara aprobado lo que no se probo.',
  verificacion,
  pendientes,
};
