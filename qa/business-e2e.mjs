...
async function createThroughWizard() {
  ...
  if (!clicked) {
    ...
    await wait(`location.pathname==='/negocio/nuevo' && document.body.innerText.includes('Tipo de negocio')`, 20_000, 'wizard');
  // FASE 2: el flujo pide el rubro PRIMERO y muestra los disenos antes de los datos.
  await clickText('Florister\u00eda', 'button');
  await clickText('Continuar');
  await wait(`!document.body.innerText.includes('Cargando dise\u00f1os') && !!document.querySelector('[data-testid="design-gallery"] article')`, 20_000, 'galeria de disenos');
  const gallery = await evaluate(`(()=>({count:document.querySelectorAll('[data-testid="design-gallery"] article').length,labels:[...document.querySelectorAll('[data-testid="design-gallery"] article h3')].map(n=>(n.textContent||'').trim()).slice(0,8)}))()`);
  if (!gallery.count) throw new Error(`Galeria sin disenos: ${JSON.stringify(gallery)}`);
  if (gallery.labels.some((label) => /FLOWERS|HAIR|_0\d|SIGNATURE/.test(label))) throw new Error(`La galeria muestra codigos tecnicos: ${JSON.stringify(gallery.labels)}`);
  evidence.designs = gallery.labels;
  results.DesignGallery = 'PASS';
  const pickedDesign = await evaluate(`(()=>{const n=[...document.querySelectorAll('[data-testid="design-gallery"] article button')].find(x=>(x.textContent||'').trim()==='Elegir');if(!n)return false;n.click();return true})()`);
  if (!pickedDesign) throw new Error('No se pudo elegir un diseno');
  await clickText('Continuar');
  await wait(`document.body.innerText.includes('Paso 3 de 5')`, 15_000, 'vista previa completa');
  const preview = await evaluate(`(()=>({scroller:!!document.querySelector('[role="dialog"] .overflow-y-auto'),renderer:!!document.querySelector('[role="dialog"] [data-business-renderer]')}))()`);
  if (!preview.scroller || !preview.renderer) throw new Error(`Vista previa incompleta: ${JSON.stringify(preview)}`);
  results.FullPreview = 'PASS';
  await evaluate(`(()=>{const n=[...document.querySelectorAll('button')].find(x=>(x.getAttribute('aria-label')||'')==='Cerrar vista previa');if(n)n.click()})()`);
  await sleep(400);
  ...
  await wait(`document.body.innerText.includes('Paso 4 de 5')`, 10_000, 'datos del negocio');
