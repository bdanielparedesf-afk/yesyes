...
export default function DesignSection({ businessId, detail, onSaved }: { businessId: string; detail: any; onSaved: (value: any) => void }) {
  ...
  const publishPreview = (nextVisual: any, nextSections: any[] = sections, templateCode?: string) => {
    previewFrame.current?.contentWindow?.postMessage({ type: 'YESYES_BUSINESS_PREVIEW', visual: nextVisual, sections: nextSections, templateCode }, window.location.origin);
  };
  useEffect(() => {
  ...
  const changeTemplate = async (id: string) => {
    ...
    try { const business = await updateBusiness(businessId, { templateId: id }); onSaved(business); setStatus('saved'); setMessage('Plantilla actualizada. Tus contenidos se conservaron.'); const selected = templates.find((template) => template.id === id); if (selected) publishPreview(visual, sections, selected.code); }
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,42%)]"><div className="space-y-5">
    ...
    <section className="rounded-2xl border bg-white p-5"><h3 className="font-bold">Diseño de tu página</h3><p className="mt-1 text-sm text-stone-500">Elige entre las plantillas y los diseños firma específicos de tu rubro.</p>{templatesStatus === 'loading' ? <p className="mt-3 text-sm text-stone-500" role="status" aria-live="polite">Cargando diseños…</p> : templatesStatus === 'error' ? <p className="mt-3 text-sm text-red-600" role="alert">No pudimos cargar los diseños. Intenta nuevamente.</p> : templates.length ? <div className="mt-3 grid max-h-[30rem] gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{templates.map((template) => <button type="button" key={template.id} onClick={() => changeTemplate(template.id)} aria-pressed={templateId === template.id} className={`min-h-12 rounded-xl border px-4 py-3 text-left text-sm font-semibold transition hover:border-stone-900 ${templateId === template.id ? 'border-stone-900 bg-stone-900 text-white' : 'bg-white text-stone-800'}`}>{templateLabel(template.code, template.name)}</button>)}</div> : <p className="mt-3 text-sm text-stone-500">No hay diseños disponibles para este rubro.</p>}<p className="mt-2 text-xs text-stone-500">Cada diseño tiene una composición propia, no solo colores distintos.</p></section>
