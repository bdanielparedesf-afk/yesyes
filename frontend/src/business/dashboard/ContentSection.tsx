...
const EMPTY_CONTENT: Record<string, { title: string; text: string; action: string }> = {
  testimonials: { title: 'Agrega la primera opinión', text: 'Muestra a tus clientes la experiencia que viven en tu negocio.', action: 'Agregar opinión' },
  faqs: { title: 'Aclara las preguntas frecuentes', text: 'Responde dudas antes de que tus clientes tengan que contactarte.', action: 'Agregar pregunta' },
  promotions: { title: 'Crea tu primera promoción', text: 'Destaca una oferta o novedades que quieras compartir.', action: 'Agregar promoción' },
  team: { title: 'Presenta a tu equipo', text: 'Genera confianza mostrando quién hace posible tu trabajo.', action: 'Agregar persona' },
};
...
export default function ContentSection({ businessId }: { businessId: string }) {
  ...
  return <div className="space-y-5">
    ...
    {!items.length ? <div className="rounded-2xl border border-dashed bg-white px-5 py-10 text-center"><b className="block text-lg">{EMPTY_CONTENT[section].title}</b><p className="mx-auto mt-2 max-w-md text-sm text-neutral-500">{EMPTY_CONTENT[section].text}</p><button type="button" onClick={() => document.querySelector<HTMLInputElement>('form input')?.focus()} className="mt-5 rounded-xl bg-stone-900 px-5 py-3 text-sm font-bold text-white">{EMPTY_CONTENT[section].action}</button></div> : <ul className="space-y-2">{items.map((item) => <li key={item.id} className="flex items-center gap-3 rounded-xl border bg-white p-3"><div className="min-w-0 flex-1"><p className="truncate font-semibold">{title(item)}</p><p className="truncate text-sm text-neutral-500">{item.role || item.description || item.answer || item.bio}</p></div><button type="button" onClick={() => { setEditId(item.id); setForm(Object.fromEntries(fields[section].map((f) => [f.key, String(item[f.key] ?? '')]))); }}>Editar</button><button type="button" className="text-red-600" onClick={() => remove(item.id)}>Eliminar</button></li>)}</ul>}
  </div>;
}
