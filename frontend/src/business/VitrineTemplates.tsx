...
export function Grid({ items, business, dark = false }: { items: any[]; business: any; dark?: boolean }) {
  ...
  return <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{items.map((item) => <article key={item.id} className={`group border p-4 ${dark ? 'border-white/10 bg-white/5' : 'border-neutral-200 bg-white shadow-sm'}`}>{productImage(item) && <img src={productImage(item)} alt={item.name} loading="lazy" className="mb-4 aspect-[4/3] w-full object-cover" />}<p className="font-bold">{item.name}</p><p className="mt-1 line-clamp-2 text-sm opacity-70">{item.shortDescription || item.description || item.bio}</p><div className="mt-4 flex justify-between gap-2"><b>{money(item)}</b><a href={buildWaLink(business.whatsapp, `Hola, quiero informaciÃ³n sobre: ${item.name}`)} target="_blank" rel="noreferrer" onClick={() => trackEvent(business.slug, 'WHATSAPP_CLICK')}>{item.cta || 'Consultar'}</a></div></article>)}</div>;
}
  ...
  return <section id="informacion" className="border-y border-stone-800 bg-stone-900">
    <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 md:grid-cols-2 md:py-20">
      {hours.length > 0 && <div>
        ...
        <h2 className="mt-3 text-3xl font-black">CuÃ¡ndo puedes visitarnos</h2>
      ...
      {(address || business?.phone || mapHref) && <div>
        <p className="flex items-center gap-2 text-xs uppercase tracking-[.4em] text-amber-400"><MapPin size={15} /> UbicaciÃ³n</p>
        <h2 className="mt-3 text-3xl font-black">EncuÃ©ntranos</h2>
        {address && <p className="mt-7 max-w-md text-lg leading-8 text-stone-200">{address}</p>}
        {business?.phone && <a className="mt-3 block w-fit text-stone-300 underline underline-offset-4" href={`tel:${business.phone}`}>TelÃ©fono: {business.phone}</a>}
        {mapHref && <a className="mt-7 inline-flex items-center gap-2 rounded-full border border-amber-400 px-6 py-3 font-bold text-amber-300 transition hover:bg-amber-400 hover:text-stone-950" href={mapHref} target="_blank" rel="noreferrer"><Navigation size={16} /> CÃ³mo llegar</a>}
      </div>}
    </div>
  </section>;
}
...
export default function VitrineTemplates({ business, services, products, gallery }: TemplateProps) {
  ...
  const photo = heroImage(business, business?.category) || gallery?.[0]?.url || productImage(products?.[0]);
  const message = waMessage(business, services?.[0] ? `quiero ${services[0].name}` : 'quiero conocer mÃ¡s');
  if (code === 'FOOD_01') return <div id="inicio" className="-mx-4 -mt-6 bg-stone-950 text-white"><Hero business={business} photo={photo} eyebrow="Cocina de autor" title="Una experiencia que empieza en la mesa." href="#catalogo" dark /><section id="catalogo" className="mx-auto grid max-w-6xl gap-10 px-4 py-20 lg:grid-cols-[.7fr_1.3fr]"><div><p className="text-xs uppercase tracking-[.4em] text-amber-400">Nuestra propuesta</p><p className="mt-5 text-stone-300">{business.description}</p></div><div><p className="text-xs uppercase tracking-[.4em] text-amber-400">MenÃº</p><div className="mt-5"><Grid items={products || []} business={business} dark /></div></div></section><Gallery gallery={gallery || []} /><RestaurantInfo business={business} /><CTABar business={business} slug={business.slug} message={message} waLabel="Pedir ahora" /></div>;
  if (code === 'BOUTIQUE_01') return <div id="inicio" className="-mx-4 -mt-6 bg-[#f7f2ea]"><header className="mx-auto grid min-h-[80vh] max-w-6xl items-center gap-8 px-4 py-16 lg:grid-cols-2"><div><p className="text-xs uppercase tracking-[.4em]">Nueva colecciÃ³n</p><h1 className="mt-4 font-serif text-6xl sm:text-8xl">{business.name}</h1><p className="mt-5 text-stone-600">{business.description}</p><a href="#catalogo" className="mt-8 inline-block rounded-full bg-stone-950 px-7 py-3 text-white">Descubrir colecciÃ³n</a></div>{photo && <img src={photo} alt={business.name} className="aspect-[4/5] h-full w-full object-cover" />}</header><section id="catalogo" className="mx-auto max-w-6xl px-4 py-20"><p className="text-center text-xs uppercase tracking-[.4em]">Piezas esenciales</p><div className="mt-8"><Grid items={products || []} business={business} /></div></section><Gallery gallery={gallery || []} /><WhatsAppButton phone={business.whatsapp} message={message} slug={business.slug} label="AsesorÃ­a de estilo" /></div>;
  if (code === 'PHOTO_01') return <div id="inicio" className="bg-neutral-950 text-white"><Hero business={business} photo={photo} eyebrow="Estudio de fotografÃ­a" title="Historias que merecen ser vistas." href="#galeria" dark /><Gallery gallery={gallery || []} /><section className="mx-auto max-w-6xl py-20"><p className="text-xs uppercase tracking-[.4em] text-cyan-300">Servicios</p><div className="mt-6"><Grid items={services || []} business={business} dark /></div></section><CTABar business={business} slug={business.slug} message={message} waLabel="Agendar sesiÃ³n" /></div>;
