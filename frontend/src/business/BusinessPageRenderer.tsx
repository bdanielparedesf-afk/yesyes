import { getNormalizedTemplateCode, hasDedicatedTemplate, isSignatureTemplate, resolveTemplate, type TemplateProps } from './registry';
...
function CategoryComposition({ business, services, products, gallery, properties, sections, cta }: TemplateProps & { sections: Array<{ id: string; enabled: boolean; order: number }>; cta: string }) {
  ...
  return <div className="space-y-10" data-business-sections={sections.map((s) => s.id).join(',')}>
    {sections.filter((s) => s.enabled).map((section) => {
      ...
      if (section.id === 'CTA') return <section key={section.id} className="text-center rounded-2xl bg-neutral-100 p-6"><p className="font-semibold mb-3">{cta}</p><a className="rounded-full bg-black text-white px-6 py-3" href={buildWaLink(business?.whatsapp, `Hola ${business?.name || ''}, quiero ${cta.toLowerCase()}.`)}>{cta}</a></section>;
      if (['DELIVERY','PICKUP','MAP','PROMOTIONS','PRICING','BOOKING','SOCIALS'].includes(section.id)) return null;
