...
export default function AdminBusinessEditor() {
  ...
  return <div className="space-y-6"><div className="flex flex-wrap gap-3 items-center justify-between"><div><h1 className="text-2xl font-bold">Editor Business</h1><p className="text-sm text-neutral-500">{business?.name || 'Cargando…'}</p></div><div className="flex gap-3"><a className="rounded-lg border px-3 py-2 text-sm" href={`/mi-negocio/${business?.slug}?preview=true`} target="_blank" rel="noreferrer">Vista previa</a><a className="rounded-lg bg-neutral-950 px-3 py-2 text-sm text-white" href={`/negocio/configuracion?id=${id}`}>Abrir panel completo</a><Link className="underline text-sm" to="/admin/negocios">Volver</Link></div></div>{message && <p className="text-sm text-neutral-700">{message}</p>}<section className="bg-white rounded-xl border p-4 space-y-3"><h2 className="font-semibold">Secciones activas y orden</h2>{capabilities.map((s, i) => <div key={s.id} className="flex items-center gap-2 border rounded-lg p-2"><span className="font-mono text-sm w-16">{s.id}</span><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={s.enabled} onChange={() => toggle(s.id)} /> Activa</label><button type="button" className="ml-auto border rounded px-2" onClick={() => move(i, -1)} aria-label={`Subir ${s.id}`}>↑</button><button type="button" className="border rounded px-2" onClick={() => move(i, 1)} aria-label={`Bajar ${s.id}`}>↓</button></div>)}<button type="button" disabled={busy} onClick={save} className="bg-black text-white rounded-lg px-4 py-2 disabled:opacity-50">{busy ? 'Guardando…' : 'Guardar capabilities'}</button></section></div>;
}
